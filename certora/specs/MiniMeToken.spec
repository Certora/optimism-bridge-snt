...
invariant allBlockNumbersAreGreaterOrEqualToCreationBlock(address user, uint256 index)
    ...
    { preserved with (env e) { require e.block.number > creationBlock(); requireInvariant blockNumberMonotonicInc(e); } }

// // all parents fromBlock should be less than the fromBlock in the cloned miniMe token (if exists).

invariant balanceOfLessOrEqToTotalSpply(address owner, uint blocknumber)
    balanceOfAt(owner, blocknumber) <= totalSupplyAt(blocknumber)
    { preserved with (env e1) { requireInvariant blockNumberMonotonicInc(e1); } }
...
rule noFeeOnTransferFrom(address alice, address bob, uint256 amount) {
    ...
    require getLatestBlockNumberByAddress(alice) < e.block.number;
    require getLatestBlockNumberByAddress(bob) < e.block.number;
...
rule noFeeOnTransfer(address bob, uint256 amount) {
    ...
    requireInvariant checkPointBlockNumberValidity(e);
    require e.block.number < max_uint128;
    require getLatestBlockNumberByAddress(e.msg.sender) < e.block.number;
    require getLatestBlockNumberByAddress(bob) < e.block.number;
